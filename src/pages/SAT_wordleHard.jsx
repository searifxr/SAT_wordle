...
function SATWordleHard({
  ...
  const winKey = `hardHasWon-${today}`;
  const hasWonToday = !!localStorage.getItem(winKey);
  ...
  const checkIfGuessed = () => {
    ...
    if(correct) {
      setTimeout(()=> {
        ...
        // Only count win if not already won today
        if (!hasWonToday) {
          // increment win count here
